...
class Learner(nn.Module):
    ...
    def __init__(self, args, config):
        ...
        pre_train_model=torch.load('/media/mcislab/gaozhi/disk_2/few-shot-meta-baseline-master/few-shot-meta-baseline-master_multiproduct_projection/save/classifier_mini-imagenet_resnet12-widecurvautre-1_lr0.01_component16_resnet12projection/epoch-55.pth')
    ...
    def generate_lr(self,per_step_task_embedding):
        ...
        x=F.sigmoid(x*0.001) # the valuable domain is [-10,10]
        x=x*10000
        #x=F.sigmoid((x-5)/2) # the valuable domain is [-10,10]
        #output=torch.zeros(x.shape[1]).cuda()
        #output[0]=(x[0,0]/2.1)+1e-2
        #output[1]=(x[0,1]/10)+3e-4
        return x
...
#('block' input_channel,output_channel,kernelsize,stride=1,padding=0,component_num,specific_num,max_pool, No.layer)
#('distance',w,h,compoent_num,all_channel,class_num)

'''
config = [
    ('block', [3, 32, 3, 1, 0, 8, 4, 2, '1']),
    ('block', [32, 32, 3, 1, 0, 8, 4, 2, '2']),
    ('block', [32, 32, 3, 1, 0, 8, 4, 2, '3']),
    ('block', [32, 32, 3, 1, 0, 8, 4, 2, '4']),
    ('distance', [3, 3, 8, 32, 5])
]

model = Learner(config)
model = model.cuda()
input_data=torch.rand(32,3,84,84).cuda()
#input_data=F.normalize(input_data,1)
label = torch.arange(4).repeat(8).cuda()
labels= label.type(torch.cuda.LongTensor).cuda()

logits=model(input_data,specific_vars=None, shared_vars=None, bn_training=True)
print('logits',logits)
loss=F.cross_entropy(logits, labels)
print('loss',loss)


specific_parameters_grad = torch.autograd.grad(loss, model.specific_parameters())
print('specific_parameters_grad',specific_parameters_grad)
specific_parameters_fast_weights = list(map(lambda p: p[1] - 0.0001 * p[0], zip(specific_parameters_grad, model.specific_parameters())))
#print('shared_parameters_fast_weights',shared_parameters_fast_weights)
'''
