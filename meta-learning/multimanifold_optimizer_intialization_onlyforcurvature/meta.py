...
class Meta(nn.Module):
    ...
    def __init__(self, args, config):
        ...
        to_optim          = [
                             {'params':self.net.regularizer.parameters(),'lr':args.meta_lr},
                             ]
    ...
    def forward(self, x_spt, y_spt, x_qry, y_qry,step,epoch):
        ...
        for i in range(task_num):
            ...
            #print('generated_lr',generated_lr)

            k_lr=[]

            for ii in range(num_k):
                k_lr.append(generated_lr[ii])
                loss_meta=loss_meta+torch.pow(generated_lr[ii]-1,2)
            ...
            if epoch>2:
                ...
                specific_k_fast_weights = list(map(lambda p: p[1] - self.args.k_lr * torch.pow(p[2],2) * p[0], zip(specific_k_grad, self.net.specific_ks(), k_lr)))
            else:
            ...
            for k in range(1, self.update_step):
                ...
                if epoch >2:
                    ...
                    specific_k_fast_weights = list(map(lambda p: p[1] - self.args.k_lr * torch.pow(p[2],2)* p[0], zip(specific_k_grad,specific_k_fast_weights, k_lr)))
                else:
                    specific_parameters_fast_weights = self.net.specific_parameters()
                    classifier_fast_weights = self.net.classifier_parameters()
                    specific_k_fast_weights =  self.net.specific_ks()
        ...
        #loss_q.backward()
        print('lr generator loss',loss_meta)
        loss_meta.backward()
