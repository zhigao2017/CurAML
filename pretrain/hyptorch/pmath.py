...
def lorenz_factor(x, *, k=1.0, dim=-1, keepdim=False):
    ...
    return 1 / torch.sqrt( (1 + k * x.pow(2).sum(dim=dim, keepdim=keepdim)).clamp_min_(1e-5))


def poincare_mean(x, dim=0, k=1.0):
    x = p2k(x, k)
    lamb = lorenz_factor(x, k=k, keepdim=True)
    mean = torch.sum(lamb * x, dim=dim, keepdim=True) / (torch.sum(lamb, dim=dim, keepdim=True)).clamp_min_(1e-5)
    mean = k2p(mean, k)
